...
- **Photography:** live and silent lots use web-sized photos in `assets/images/lots-cropped/`. Site photography (hero, portraits, illuminations) is in `assets/images/site/`. Live Lot 07 is a "Coming Soon" placeholder (no image, no details) until the client supplies the replacement piece; former Silent Lot 07 was withdrawn and removed, so silent numbering skips 07.
...
- **Menu page:** `menu.html` shows the printed dinner card (`assets/images/site/menu.jpg`), rendered from `assets/More Tupa Website Black Direction pages/Tupa Menu Card artwork.png`. Keep the `alt` text in step with the artwork so a screen reader still gets the courses.
...
11. Paragraph replacing "Founded in 1856…" on The Cause page (impact language and Alfajiri copy are live)
